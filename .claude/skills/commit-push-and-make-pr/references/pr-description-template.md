...
- **No code snippets** — _unless_ the change introduces a new pattern the whole codebase should adopt going forward (a new helper, a new convention, a new error-handling shape). Then exactly one snippet, captioned `// new pattern to follow:`. Tweaks, fixes, refactors, and renames never need a snippet.
