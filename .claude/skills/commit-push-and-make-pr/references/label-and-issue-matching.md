...
`curl -s "https://api.github.com/repos/<owner>/<repo>/labels?per_page=100"` — captured in Phase 1 (unauthenticated GitHub REST API, no `gh` needed). **This is the only valid label source.** If the call fails (private repo, rate limit, offline), treat the repo as having no labels — skip labeling silently, don't invent any.
...
- Match **case-insensitively**, but apply the **exact existing casing** in the compare URL's `labels=` query param.
- If a candidate label isn't in the catalog → drop it. Never invent one.
- 1–4 labels total. More than 4 dilutes the signal.
- A label that doesn't exactly match an existing repo label is silently ignored by GitHub's prefill form (not an error, not auto-created) — but still only pass confirmed catalog labels so the confirm block in Phase 7 doesn't promise something that won't stick.
...
`curl -s "https://api.github.com/repos/<owner>/<repo>/issues?state=open&per_page=50"` — captured in Phase 1 (unauthenticated GitHub REST API). This endpoint also returns PRs; skip any entry that has a `pull_request` key. If the call fails, treat as no open issues — skip issue-linking silently.
...
Labels catalog → `bug`, `feature`, `documentation`, `area:billing`, `needs review`, `breaking-change`.
