...
6. **Cross-layer feature slice.** A new feature touching schema + server fn + UI is one _story_; if commits are small, group as schema-then-server-then-UI (three commits). If the slice is small, one commit is fine.
